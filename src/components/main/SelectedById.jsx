...
function SelectedById() {
  const [productData, setProductData] = useState({totalPages: 0, products: []});
        ...
        <div className="px-2 mdl:flex gap-2 w-full">
          ...
          <div className="content">
            ...
            <div className="products-category ">
              ...
              <div className="products-lists grid place-content-center mdl:place-content-end gap-5 custom:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 flex-wrap">
                {productData
                  ? productData.products.map((item,i) => {
                      return (
                        <Link to={`/details/${item.id}`} key={i} className="bg-white border-[1px] h-full rounded-md flex flex-col items-center justify-center w-full lgx:w-[200px]">
                          ...
                          <div className="flex justify-between items-center w-[110px]">
                            <button className="text-[#ff8230] text-[2.3rem] font-bold">
                              -
                            </button>
                            <span>1</span>
                            <button className="text-[#ff8230] text-[2.2rem]  font-bold">
                              +
                            </button>
                          </div>
                          <button className="bg-[#ff8230] hover:bg-[#e4742a] transition duration-200 text-white rounded-full w-[100px] h-[35px] mb-10">
                            Sebete at
                          </button>
                        </Link>
                      );
              ...
              <div className="flex gap-3 justify-center py-6 max-w-[800px]">
                {/* {Array(productData && productData.totalPages)
                  .fill("")
                  .map((_, i) => (
                    <button key={i+1} onClick={() =>{
                      ...
                      type="button"
                      title={`Page ${i+1}` } 
                      className="w-8 h-8 text-sm font-semibold border rounded shadow-md text-[#ff8230] hover:text-white bg-white hover:bg-[#ff8230] border-[#ff8230]"
                    >
                      {i + 1}
                    </button>
                  ))} */}
                  <Pagination
                   current={page}
                   total={productData?.totalPages * 10}
                   pageSize={10} 
