  ...
  const updateCount = (id, amount) => {
    setProducts((prevProducts) =>
      prevProducts.map((product) =>
        product.id === id
          ? { ...product, count: product.count + amount < 1 ? 1 : product.count + amount }
          : product
      )
    );
  };
          ...
          {products.map((item) => (
            <SwiperSlide key={item.id} className="relative">
              <div className="bg-white border-[1px] h-[350px] rounded-md flex flex-col items-center justify-center w-full lgx:w-[190px]">
                ...
                <Link to={`${item.id}`} className="relative w-[100px] h-[140px]">
                ...
                <div className="flex justify-between items-center w-[110px]">
                  <button onClick={() => updateCount(item.id, -1)} className="text-[#ff8230] text-[2.3rem] font-bold">-</button>
                  <span>{item.count}</span>
                  <button onClick={() => updateCount(item.id, 1)} className="text-[#ff8230] text-[2.2rem] font-bold">+</button>
                </div>
