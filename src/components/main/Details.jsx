        ...
        <div className="bg-gray-100">
            <nav aria-label="breadcrumb" className="w-full p-4">
                <ol className="flex h-8 space-x-2 text-[.8rem]">
                    ...
                    <li className="flex items-center space-x-1">
                        <span className="text-gray-600">/</span>
                        <a rel="noopener noreferrer" href="#" className="flex items-center px-1 capitalize hover:no-underline cursor-default">Current</a>
                    </li>
                </ol>
            </nav>
            <div className="flex flex-col p-6 space-y-6 overflow-hidden rounded-lg shadow-md">
                {prodDetails ? (
                    <div key={prodDetails.id} className="mx-auto w-full flex flex-col md:flex-row md:gap-6">
                        <img src={prodDetails.img[0]} alt={prodDetails.name} className="object-cover custom:object-contain w-full mb-4 h-full sm:h-96" />
                        <div>
                            <h2 className="mb-1 text-xl font-semibold">{prodDetails.name}</h2>
                            <Rate />
                            <p className="text-sm dark:text-gray-600">{prodDetails.description}</p>
                            {prodDetails.discount ?
                                <div className="flex gap-2">
                                    <p className="text-3xl font-semibold py-2 text-[#ff8300]">
                                        {(prodDetails.price - prodDetails.totalPrice).toFixed(2)} AZN
                                    </p>
                                    <p className="text-sm font-semibold py-2 text-gray-400 line-through">
                                        {prodDetails.price} AZN
                                    </p>
                                </div>
                                : <p className="text-3xl font-semibold py-2 text-[#ff8300]">{prodDetails.price} AZN</p>
                            }
                            <div className="flex py-6 justify-between mx-auto md:mx-0 items-center w-[110px]">
                                <button onClick={() => count > 0 ? setCount(count - 1) : 1} className="text-[#ff8230] text-[2.3rem] font-bold">-</button>
                                <span>{count}</span>
